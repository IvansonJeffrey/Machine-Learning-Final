...
def visualize_parsed_regions(
    ...
    Args:
        ...
        confidences: Optional dict mapping region names to confidence values (0.0 to 1.0)
                     If not provided, will calculate based on region coverage
    """
    ...
    # Right: Clean legend panel
    ax2 = fig.add_subplot(gs[0, 2])
    ax2.axis('off')  # Hide axes for legend panel
    
    # Create detailed legend with color squares and hexcodes
    legend_data = []
    for region_name, overlay_color in region_overlay_colors.items():
        if region_name in masks and masks[region_name].sum() > 0:
            ...
            # Get extracted color information
            color_info = extracted_colors.get(region_name, {})
            hex_code = color_info.get('hex', None)
            color_name = color_info.get('name', None)
            rgb_values = color_info.get('rgb', None)
            
            # Get confidence for this region
            confidence = confidences.get(region_name, 0.0)
            
            legend_data.append({
                'region': region_name,
                'overlay_color': overlay_color,
                'extracted_color': rgb_values,
                'hex_code': hex_code,
    ...
    # Create clean legend panel on the right
    if legend_data:
        # Title for legend panel
        ax2.text(0.5, 0.98, "Region Details", 
                fontsize=18, fontweight='bold', ha='center', va='top',
                transform=ax2.transAxes)
        
        # Start position for legend items
        start_y = 0.90
        item_height = 0.25  # More space between items
        square_size = 0.08  # Larger color squares
        square_x = 0.05  # Left margin for squares
        text_x = 0.20  # Text starts after squares
        
        for idx, data in enumerate(legend_data):
            y_pos = start_y - (idx * item_height)
            
            # Region name header
            ax2.text(text_x, y_pos, f"{data['region'].capitalize()}", 
                    fontsize=16, fontweight='bold', ha='left', va='top',
                    transform=ax2.transAxes)
            
            y_offset = y_pos - 0.04
            
            # Draw extracted color square (if available)
            if data['extracted_color']:
                extracted_rgb = [c / 255.0 for c in data['extracted_color']]
                extracted_square = mpatches.Rectangle(
                    (square_x, y_offset - square_size), square_size, square_size,
                    facecolor=extracted_rgb, edgecolor='black', linewidth=2,
                    transform=ax2.transAxes
                )
